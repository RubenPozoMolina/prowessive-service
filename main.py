...
@app.get("/items/", response_model=List[schemas.Item], tags=["users"])
...
@app.get("/projects/{project_name}", response_model=schemas.Project, tags=["projects"])
...
@app.delete("/projects/{project_name}", response_model=schemas.Project, tags=["projects"])
def delete_project(project_name: str, db: Session = Depends(get_db)):
    project = crud.delete_project(db, project_name)
    return project
